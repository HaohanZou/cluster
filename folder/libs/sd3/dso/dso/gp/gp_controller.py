...
class GPController:
    ...
    def __call__(self, actions):
        ...
        '''
        with Pool() as pool:
            # Create a generator for the generation indices
            gen_indices = list(range(self.generations))
            # Progress bar wrapping the parallel pool
            for nevals in tqdm(pool.imap(self.run_generation, gen_indices), total=len(gen_indices)):
